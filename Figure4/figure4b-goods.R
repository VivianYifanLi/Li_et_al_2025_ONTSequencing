...
goods_run1=df_prok_noc1 %>% 
  ...
  ggplot(aes(x=n_seqs, y=goods, label=sampleID)) + theme_bw() + geom_point() + geom_text(vjust=0, hjust=-0.3, size=5) + scale_y_continuous(name="Good's coverage index (%)", limits=c(82, 100), n.breaks=8) + theme(text=element_text(size=15)) + xlab("Number of reads") + theme(text=element_text(size=20))
goods_run1
